...
from npupy_xdna.templates.compute_pool_8core import ComputePool8CoreTemplate, SIZES_8CORE
...
SIZES = SIZES_8CORE
JSONL_PATH = Path(__file__).parent.parent / "results" / "timings" / "compute_pool_8core.jsonl"
N_WARMUP = 5
N_ITERS = 10
...
def main() -> None:
    ...
    print("=" * 60)
    print("ComputePool8CoreTemplate Characterization Sweep")
    print(f"Sizes: {SIZES}")
    ...
    print(f"\nDone. {len(rows)} rows -> {JSONL_PATH}")
    print(f"\n{'Size':>10}  {'NPU(us)':>10}  {'CPU(us)':>10}  {'BW(GB/s)':>10}  {'Speedup':>8}")
    print("-" * 56)
    for r in rows:
        print(
